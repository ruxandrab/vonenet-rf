...
parser.add_argument('--in_path', required=True,
                    help='path to Tiny ImageNet folder that contains train and val folders')
parser.add_argument('-o', '--output_path', default='model/Run1',
...
parser.add_argument('--optimizer', choices=['stepLR', 'plateauLR'], default='plateauLR',
                    help='Optimizer')
...
def train(save_train_epochs=.2,  # how often save output during training
    ...
    trainer = TinyImageNetTrain(model)
    validator = TinyImageNetVal(model)
            ...
            if FLAGS.output_path is not None:
                ...
                if save_model_steps is not None:
                    if global_step in save_model_steps:
                        if os.path.exists(os.path.join(FLAGS.output_path, f'epoch_{epoch-1:02d}.pth.tar')):
                            os.remove(os.path.join(FLAGS.output_path, f'epoch_{epoch-1:02d}.pth.tar'))
                        torch.save(ckpt_data, os.path.join(FLAGS.output_path,
...
class TinyImageNetTrain(object):
    ...
    def data(self):
        dataset = torchvision.datasets.ImageFolder(
            ...
            torchvision.transforms.Compose([
                ...
                torchvision.transforms.RandomAffine(degrees=(-30,+30),translate=(0.05, 0.05),scale=(1,1.2)),
