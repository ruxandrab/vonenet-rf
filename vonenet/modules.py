...
class GFB(nn.Module):
    ...
    def initialize(self, sf, theta, sigx, sigy, phase, color):
        for i in range(self.out_channels):
            self.weight[i, color[i]] = gabor_kernel(frequency=sf[i], sigma_x=sigx[i], sigma_y=sigy[i],
                                                             theta=theta[i], offset=phase[i], ks=self.kernel_size[0])
        self.weight = nn.Parameter(self.weight, requires_grad=False)


class VOneBlock(nn.Module):
    def __init__(self, sf, theta, sigx, sigy, phase, color,
                 k_exc=25, noise_mode=None, noise_scale=1, noise_level=1,
                 simple_channels=256, complex_channels=256, ksize=31, stride=2, input_size=64):
        super().__init__()
